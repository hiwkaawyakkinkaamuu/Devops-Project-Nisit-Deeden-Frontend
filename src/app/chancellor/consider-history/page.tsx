...
import {
  ...
  ArrowUpDown, ArrowUp, ArrowDown, ChevronDown, History
} from "lucide-react";
...
// 🌟 Custom Dropdown Component (Theme Purple) 🌟
    ...
    return (
        <div className={`relative w-full ${className}`} style={{ zIndex: isOpen ? 40 : 1 }} ref={dropdownRef}>
            <div 
                ...
                className={`flex items-center justify-between w-full pl-11 pr-4 py-3.5 bg-transparent/80 backdrop-blur-sm border rounded-2xl cursor-pointer transition-all duration-300 shadow-sm
                    ${isOpen ? 'border-purple-400 ring-4 ring-purple-500/10' : 'border-slate-200/80 hover:border-slate-300'}
                `}
            >
                <Icon className={`w-4 h-4 absolute left-4 top-3.5 transition-colors ${isOpen ? 'text-purple-500' : 'text-slate-400'}`} />
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute top-[calc(100%+8px)] left-0 w-full bg-transparent/95 backdrop-blur-xl border border-slate-100 rounded-2xl shadow-2xl py-2 max-h-60 overflow-y-auto z-[50] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 hover:[&::-webkit-scrollbar-thumb]:bg-slate-300"
                    >
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // ดึงเฉพาะสถานะ 13 (ลงนามโดยอธิการบดีเสร็จสิ้นแล้ว) มาแสดงในหน้าประวัติ
        const TARGET_STATUS_ID = 13; 
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-fade-in-up relative z-[60]" style={{ animationDelay: '100ms' }}>
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10"><Search className="h-4 w-4 text-slate-400 group-focus-within:text-purple-500 transition-colors" /></div>
              <input 
                type="text" 
                placeholder="ค้นหาชื่อ หรือ รหัสนิสิต" 
                value={searchTerm} 
                onChange={(e) => setSearchTerm(e.target.value)} 
                className="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3.5 pl-11 text-sm font-medium focus:ring-4 focus:ring-purple-500/10 focus:border-purple-400 outline-none transition-all shadow-sm placeholder:text-slate-400 text-slate-800 relative z-10" 
              />
            </div>
        ...
        <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 overflow-visible animate-fade-in-up relative z-10" style={{ animationDelay: '150ms' }}>
              ...
              <thead>
                <tr className="bg-slate-50 text-slate-500 text-[11px] font-extrabold uppercase tracking-widest border-b border-slate-200">
                  ...
                  <th className="p-6 cursor-pointer hover:bg-slate-100 transition-colors text-center w-[20%]" onClick={() => handleSort('latest_update')}>
                     <div className="flex items-center justify-center gap-1.5">วันที่ลงนาม {sortConfig.key === 'latest_update' ? (sortConfig.direction === 'asc' ? <ArrowUp className="w-3.5 h-3.5 text-purple-500"/> : <ArrowDown className="w-3.5 h-3.5 text-purple-500"/>) : <ArrowUpDown className="w-3.5 h-3.5 text-slate-300"/>}</div>
                  </th>
                    ...
                    <td colSpan={5} className="p-20 text-center">
                      <div className="flex flex-col items-center justify-center text-slate-400">
                        <div className="bg-purple-50 p-5 rounded-full mb-4 shadow-sm border border-purple-100">
                          <History className="w-12 h-12 text-purple-300" strokeWidth={1.5} />
                        </div>
                        <p className="text-xl font-bold text-slate-700">ไม่มีประวัติการลงนาม</p>
                        <p className="text-sm mt-2 font-medium text-slate-500">ยังไม่มีรายการที่ได้รับการอนุมัติขั้นสุดท้ายจากคุณ</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                        ...
                        >
                          ...
                          <td className="p-6 text-center align-middle">
                            {isOrg ? (
                                <span className="text-purple-500 font-bold bg-purple-50 px-3 py-1 rounded-md border border-purple-100">องค์กรภายนอก</span>
                            ) : (
                          ...
                          <td className="p-6 text-center align-middle">
                              <span className="inline-flex items-center px-3.5 py-1.5 rounded-xl text-xs font-bold shadow-sm bg-purple-100 text-purple-700 border border-purple-200">
                                  <CheckCircle2 className="w-3.5 h-3.5 mr-1.5" strokeWidth={3} /> ลงนามสำเร็จ
                              </span>
                          </td>
          ...
          <div className="bg-slate-50 border-t border-slate-200 p-5 flex flex-col md:flex-row justify-between items-center gap-4 rounded-b-[32px]">
            ...
            <div className="text-[13px] text-slate-600 font-bold bg-white px-5 py-2.5 rounded-xl border border-slate-200 shadow-sm">
                พบประวัติการลงนามทั้งหมด <span className="text-purple-600 text-[14px] ml-1">{processedData.length}</span> รายการ
            </div>
          </div>
        </div>
